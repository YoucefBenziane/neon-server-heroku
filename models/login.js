...
const login = (request, response) => {
    pool.query('SELECT * FROM Logins ORDER BY price ASC', (error, results) => {
        if (error) {
            throw error
        }
...
const logout = (request, response) => {
    ...
    pool.query('SELECT * FROM Logins WHERE name = $1', [name], (error, results) => {
        if (error) {
            throw error
        }
...
const createLogin = async (request, response) => {
    ...
    const id = 0
    pool.query('INSERT INTO Logins (name, price, maxCount, expirationDate, creationDate, useCount, disabled) VALUES ($1, $2, $3, $4, $5, $6, $7)',
...
const updateLogin = (request, response) => {
    ...
    pool.query(
        'UPDATE Logins SET name = $1, price = $2, maxCount = $3, expirationDate = $4, disabled = $5, useCount = $6  WHERE name = $1',
        [name, price, maxcount, expirationdate, disabled, usecount],
        (error, results) => {
            if (error) {
                throw error
            }
            response.status(200).send(`Logins modified with ID: ${name}`);
        }
    )
};
...
const deleteLogin = (request, response) => {
    ...
    pool.query('DELETE FROM Logins WHERE name = $1', [name], (error, results) => {
        if (error) {
            throw error
        }
        response.status(200).send(`Logins deleted with ID: ${name}`);
    })
};
