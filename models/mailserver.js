...
const getCoupons = (request, response) => {
    pool.query('SELECT * FROM coupons ORDER BY price ASC', (error, results) => {
        if (error) {
            throw error
        }
...
const createCoupon = async (request, response) => {
    ...
    const id = 0
    pool.query('INSERT INTO coupons (name, price, maxCount, expirationDate, creationDate, useCount, disabled) VALUES ($1, $2, $3, $4, $5, $6, $7)',
...
const updateCoupon = (request, response) => {
    ...
    pool.query(
        'UPDATE coupons SET name = $1, price = $2, maxCount = $3, expirationDate = $4, disabled = $5, useCount = $6  WHERE name = $1',
        [name, price, maxcount, expirationdate, disabled, usecount],
        (error, results) => {
            if (error) {
                throw error
            }
            response.status(200).send(`coupons modified with ID: ${name}`);
        }
    )
};
